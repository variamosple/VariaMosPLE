...
function isIterationResult(result: Array<any> | object | boolean): boolean {
  return Array.isArray(result) && result.some((elem) => Array.isArray(elem) && elem.length === 2);
}
...
export default function QueryResult(
  ...
  const visibleResults = useMemo(() => {
    if (Array.isArray(result) && isIterationResult(result)) {
      ...
      return result
        .map((elem) => [elem[0].replace("UUID_", "").replaceAll("_","-"), elem[1]])
  ...
  const handleIterationQueryVisualization = () => {
    ...
    (visibleResults as Array<Array<string | boolean>>).forEach(([elem_id, _]) => {
      ...
      if (cell) {
        const overlayFrame = new mx.mxCellOverlay(new mx.mxImage("images/models/Eo_circle_red_white_no-entry.svg", 24, 24), 'Failed Query');
