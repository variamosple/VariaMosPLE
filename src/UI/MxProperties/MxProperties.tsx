...
export default class MxProperties extends Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.projectService_addSelectedElementListener = this.projectService_addSelectedElementListener.bind(this);
  ...
  componentDidMount() {
    ...
    me.props.projectService.addSelectedElementListener(this.projectService_addSelectedElementListener);
  }

  renderProperties() {
    let ret = []
    if (this.currentObject) {
      let languageDefinition: any = this.props.projectService.getLanguageDefinition("" + this.currentModel?.name);
      if (languageDefinition) {
        let elementDef: any = languageDefinition.abstractSyntax.elements[this.currentObject.type];
    ...
    switch (property.type) {
      case "Select":
        ...
        for (let i = 0; i < property.options.length; i++) {
          ...
          if (option==value) {
            options.push(<option  data-name={property.name} value={option} selected>{option}</option>)
          }else{ 
            options.push(<option  data-name={property.name} value={option}>{option}</option>)
          }
        }
        control = <select data-name={property.name} onChange={this.input_onChange}>{options}</select>;
        break;
      case "Text":
        control = <textarea data-name={property.name} value={value} onChange={this.input_onChange} />;
        break;
      case "Boolean":
        if (value == true) {
          control = <input data-name={property.name} type="checkbox" checked={true} onChange={this.checkBox_onChange} />;
        } else {
          control = <input  data-name={property.name} type="checkbox" onChange={this.checkBox_onChange} />;
        }
        break;
      default:
        control = <input type="text" data-name={property.name} onChange={this.input_onChange} value={value} />;
        break;
    }
    return (
      <div>
